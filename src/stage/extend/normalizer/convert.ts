// 文件：src/old_design/normalizer/index.ts
// 作用：将上游JSON Schema结构转换为内部Schema模型，供旧管道使用

import type { Schema, ObjectSchema, ArraySchema, PrimitiveSchema } from '../../../types.js';

/**
 * 将JSON Schema转换为内部Schema模型
 * @param input JSON Schema输入
 * @returns 内部Schema模型或undefined
 */
 ...
 * 从示例数据推断Schema
 * @param example 示例数据
 * @returns 推断出的Schema或undefined
 */
export function inferSchemaFromExample(example: any): Schema | undefined {
  if (example === null || example === undefined) return { kind: 'null' };
  if (Array.isArray(example)) {
    const schemas = example.map(inferSchemaFromExample).filter(Boolean) as Schema[];
    const element = schemas.length ? schemas[0] : undefined;
  ...
  switch (typeof example) {
    case 'string':
      return { kind: 'string' };
    case 'number':
      return Number.isInteger(example) ? { kind: 'integer' } : { kind: 'number' };
    case 'boolean':
      return { kind: 'boolean' };
    case 'object': {
      const properties: Record<string, Schema> = {};
      for (const key of Object.keys(example)) {
        const schema = inferSchemaFromExample((example as any)[key]);
 ...
 * 归一化扩展接口
 */
export interface NormalizerExtension {
  /**
   * 执行归一化操作
   * @param input 输入数据
   * @returns 归一化后的Schema或undefined
   */
  normalize(input: any): Schema | undefined;
  /** 扩展名称 */
  name: string;
}